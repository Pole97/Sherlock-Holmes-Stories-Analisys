...
<h4> Short summary of the novels: </h4>
...
For the sake of completeness we pllay the tf-idf technique also to the bigrams and we see that most of them contains the unigrams that we found preiously.
...
Let's plot the top 6 word that correlate to *holmes*, *house*, *sir*, and *time*, that are the four most frequent words. 
...
We begin by providing a technical overview of the STM model. Like other topic models, the STM is a generative model of word counts. That means we define a data generating process for each document and then use the data to find the most likely values for the parameters within the model. This method is used for extracting the per-topic-per-word probabilities, called $\beta$ (“beta”), from the model.

Now let's visualize 10 words that are the most common in each topic that were extracted from the books. Meanwhile the second topic contains mostly common and neutral words, the first seems to refer to the book "The Hund of the Baskervilles"
...
In this case, one novel is strongly associated with the first and more specific topic meanwhile the second and more general topic is associated with three books. In any case, this is how we interpret these gamma probabilities; they tell us which topics are coming from which documents.
